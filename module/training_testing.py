...
def training(model, train_dset, valid_dset, device, verbose, hyperparameter):
    """
    ...
    plt.legend()
    plt.savefig('./curve/accuracy_curve.png')
    ...
    plt.legend()
    plt.savefig('./curve/loss_curve.png')
...
def testing(model, test_dset, device):
    ...
    with torch.no_grad():
        ...
        print(f"Test Precision: {metrics.recall_score(whole_answer, whole_model_suggestion)}")